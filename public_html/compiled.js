...
},{"@thaerious/nidget":14,"jquery":1}],3:[function(require,module,exports){
...
/**
 * Singlton class to providing functionality to DragNidgets and DropNidgets.
...
},{"./NidgetElement":8,"jquery":3}],7:[function(require,module,exports){
"use strict";
...
class MouseUtilities {
    ...
    onMouseMove(event) {
        ...
        let deltaX = this.lastX - event.clientX;
        let deltaY = this.lastY - event.clientY;
        this.lastX = event.clientX;
        this.lastY = event.clientY;

        // set the element's new position:
        this.attachedElement.style.top = (this.offsetTop - deltaY) + "px";
        this.attachedElement.style.left = (this.offsetLeft - deltaX) + "px";
    }
}
 ...
 * object should only be called by the NidgetElement object.
 */
class NidgetElement extends HTMLElement{    
     ...
     * @return {nm$_NidgetElement.NidgetElement}
     */
    constructor(){
        super();
    }
    ...
    connectedCallback(){
        if (this.hasAttribute(NidgetElement.interfaceAttribute)) this.applyInterfaces();
        if (this.hasAttribute(NidgetElement.srcAttribute)) this.retrieveSource();
        if (this.hasAttribute(NidgetElement.templateAttribute)) this.applyTemplate();
    }
    
    applyTemplate(){
        let templateId = this.getAttribute(NidgetElement.templateAttribute);
        let template = document.getElementById(templateId);
        
        if (!template) throw new Error("Template '" + templateId + "' not found.");
        if (template.name !== "template")  throw new Error("Element with id '" + templateId + "' is not a template.");
    ...
    retrieveSource(map = {}){
        let src = this.getAttribute(NidgetElement.srcAttribute);        
        let ele = FileOperations.loadDOMElement(src, map);
        const shadowRoot = this.attachShadow({mode: 'open'}).appendChild(ele);        
    }
    
    applyInterfaces(){
        for (let nInterface of this.interfaces()){            
            if (typeof NidgetElement.interfaces[nInterface] === "function"){
                window.addEventListener("load", ()=>NidgetElement.interfaces[nInterface](this));
            }
        }
    }
    
    /**
     * Place this nidget relative to it's parent.
     * @return {undefined}
     */
    place(){
        for (var i = 0; i < arguments.length; i++) {
            this.__place(arguments[i]);
        }
    }
    ...
    left(value){
        console.log(`nidget.left(${value})`);
        console.log(this);
        $(this).css("left", value);
    }
    
    top(value){
        console.log(`nidget.top(${value})`);
        console.log(this);
        $(this).css("top", value);
    }    
    
    text(selector, value){
        if (value === undefined) return $(this.shadowRoot).find(selector).text();
        $(this.shadowRoot).find(selector).text(value);
    }
    
    interfaces(){
        let interfaceText = $(this).attr(NidgetElement.interfaceAttribute);
...
NidgetElement.prefix = "data-nidget";
NidgetElement.elementAttribute = "data-nidget-element";
NidgetElement.srcAttribute = "src";
NidgetElement.nameAttribute = "name";
NidgetElement.interfaceAttribute = "interfaces";
NidgetElement.templateAttribute = "template";

NidgetElement.interfaces = {
...
console.log("register nidget-element");
window.customElements.define('nidget-element', NidgetElement);
module.exports = NidgetElement;


},{"./FileOperations":6,"./nidget-interfaces/Drag":10,"./nidget-interfaces/Drop":11,"./nidget-interfaces/Mouse":12,"./nidget-interfaces/Movable":13,"jquery":3}],9:[function(require,module,exports){
"use strict";
const $ = window.$ ? window.$ : require("jquery");
 ...
 * - prefix-idle.png
 */
class ButtonNidget extends Nidget {

    connectedCallback() {
        super.connectedCallback();
        this.shadow = $("<div></div>")[0];

        this.imgHover = $("<img class='imgHover'/>");
        this.imgDisabled = $("<img class='imgDisabled'/>");
        this.imgPress = $("<img class='imgPress'/>");
        this.imgIdle = $("<img class='imgIdle'/>");

        this.state = "idle";
        if (this.disabled()) {
            $(this.imgHover).hide();
            $(this.imgDisabled).show();
            $(this.imgPress).hide();
            $(this.imgIdle).hide();
        } else {
            $(this.imgHover).hide();
            $(this.imgDisabled).hide();
            $(this.imgPress).hide();
            $(this.imgIdle).show();
        }

        $(this.shadow).append(this.imgHover);
        $(this.shadow).append(this.imgDisabled);
        $(this.shadow).append(this.imgPress);
        $(this.shadow).append(this.imgIdle);

        let prefix = $(this).attr("prefix");
        let suffix = $(this).attr("suffix");
        if (!suffix) suffix = ".png";

        $(this.imgHover).attr("src", prefix + "-hover" + suffix);
        $(this.imgDisabled).attr("src", prefix + "-disabled" + suffix);
        $(this.imgPress).attr("src", prefix + "-press" + suffix);
        $(this.imgIdle).attr("src", prefix + "-idle" + suffix);

        $(this.shadow).find("img").css("position", "absolute");
        $(this.shadow).find("img").css("box-sizing", "border-box");
        $(this.shadow).find("img").css("height", "100%");
        $(this.shadow).find("img").css("width", "100%");
        $(this.shadow).find("img").css("cursor", "pointer");

        this.addEventListener("mouseenter", this.mouseEnter);
        this.addEventListener("mouseout", this.mouseLeave);
        ...
        this.attachShadow({mode: 'open'}).appendChild(this.shadow);
    }
    ...
    set hoverImage(img) {
        $(this.imgHover).attr("src", img);
    }

    set disabledImage(img) {
        $(this.imgDisabled).attr("src", img);
    }

    set pressImage(img) {
        $(this.imgPress).attr("src", img);
    }

    set idleImage(img) {
        return $(this.imgIdle).attr("src");
    }

    get hoverImage() {
        return $(this.imgHover).attr("src");
    }

    get disabledImage() {
        return $(this.imgDisabled).attr("src");
    }

    get pressImage() {
        return $(this.imgPress).attr("src");
    }

    get idleImage() {
        $(this.imgIdle).attr("src");
    }
}
;

window.customElements.define('nidget-button', ButtonNidget);
module.exports = ButtonNidget;


},{"../NidgetElement":8,"jquery":3}],10:[function(require,module,exports){
"use strict";
...
},{"../DragHandler":5}],11:[function(require,module,exports){
"use strict";
const dragHandler = require("../DragHandler").instance;
...
},{"../DragHandler":5,"../MouseUtilities":7}],12:[function(require,module,exports){
"use strict";
const MouseUtilities = require("../MouseUtilities");
...
},{"../MouseUtilities":7}],13:[function(require,module,exports){
"use strict";
